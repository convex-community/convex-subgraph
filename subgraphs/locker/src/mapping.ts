import { KickReward, RewardAdded, RewardPaid, Staked, Withdrawn } from '../generated/CvxLocker/CvxLocker'
import { AddedReward, Lock, ReceivedReward, Withdrawal } from '../generated/schema'
import { getUser } from './services/users'
import {
  getEventId,
  updateAggregatedLocks,
  updateAggregatedRewards,
  updateAggregatedWithdrawals,
...
export function handleKickReward(event: KickReward): void {
  ...
  reward.amountUSD = getUsdRate(CVX_ADDRESS).times(reward.amount.toBigDecimal().div(exponentToBigDecimal(cvx.decimals)))
  reward.token = cvx.id
  reward.time = event.block.timestamp
  reward.kickReward = true
  reward.save()

  updateAggregatedRewards(reward.time, cvx, reward.amount, reward.amountUSD)
}
...
export function handleWithdrawn(event: Withdrawn): void {
  ...
  withdrawal.amountUSD = getUsdRate(CVX_ADDRESS).times(
    withdrawal.amount.toBigDecimal().div(exponentToBigDecimal(cvx.decimals))
  )
  withdrawal.time = event.block.timestamp
  user.totalLocked = user.totalLocked.minus(withdrawal.amount)
  user.save()
  withdrawal.save()
  updateAggregatedWithdrawals(withdrawal.time, withdrawal.amount, withdrawal.amountUSD)
}
